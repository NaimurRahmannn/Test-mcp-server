...
class ValidateBearerTokenTests(unittest.TestCase):
    def test_accepts_matching_token(self) -> None:
        self.assertTrue(validate_bearer_token("secret", "secret"))

    def test_rejects_empty_or_different_token(self) -> None:
        self.assertFalse(validate_bearer_token("", "secret"))
        self.assertFalse(validate_bearer_token("wrong", "secret"))
